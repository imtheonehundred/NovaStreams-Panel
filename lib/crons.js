...
async function runTmdbSync() {
  ...
  const lang =
    ((await dbApi.getSetting('tmdb_language')) || 'en').trim() || 'en';
  ...
  const tasks = [];
  for (const row of needMovies)
    tasks.push({ kind: 'movie', id: row.id, tmdbId: row.tmdb_id });
  for (const row of needSeries)
    tasks.push({ kind: 'series', id: row.id, tmdbId: row.tmdb_id });
  ...
  for (const t of tasks) {
    try {
      ...
      fail += 1;
      console.error(
        `[CRONS] TMDb sync ${t.kind} id=${t.id} tmdb=${t.tmdbId}:`,
        e.message || e
      );
    }
  }
  ...
  if (tasks.length) {
    console.log(
      `[CRONS] TMDb metadata sync: ${ok} updated, ${fail} failed (${tasks.length} attempted)`
    );
  }
}
...
function startCrons() {
  cron.schedule('* * * * *', async () => {
    try {
      ...
      const expired = await query(
        'SELECT id FROM `lines` WHERE exp_date IS NOT NULL AND exp_date < FROM_UNIXTIME(?) AND enabled = 1',
        [now]
      );
      ...
      if (expired.length) {
        console.log(
          `[CRONS] Line expiry: disabled ${expired.length} expired line(s)`
        );
      }
    } catch (e) {
      ...
      if (typeof epgService.refreshAllSources === 'function') {
        ...
        const errPart =
          result.errors && result.errors.length
            ...
            : '';
        console.log(
          `[CRONS] EPG refresh (all sources): inserted=${result.inserted} sources=${result.sources}${errPart}`
        );
      } else {
        ...
        for (const source of sources) {
          try {
            ...
            console.error(
              `[CRONS] EPG refresh source id=${source.id} failed:`,
              e.message || e
            );
          }
        }
  ...
  cron.schedule('0 2 * * *', async () => {
    try {
      if (!(await acquireLock('cron:daily_cleanup', 300))) {
        console.log('[CRONS] Daily cleanup skipped - already running');
        return;
      }
      const thirtyDaysAgo = Math.floor(Date.now() / 1000) - 30 * 86400;
      const la = await execute(
        'DELETE FROM lines_activity WHERE COALESCE(date_end, date_start) < FROM_UNIXTIME(?)',
        [thirtyDaysAgo]
      );
      const qoe = await execute(
        'DELETE FROM qoe_metrics WHERE created_at < DATE_SUB(NOW(), INTERVAL 7 DAY)'
      );
      const pl = await execute(
        'DELETE FROM panel_logs WHERE created_at < DATE_SUB(NOW(), INTERVAL 90 DAY)'
      );
      ...
      await releaseLock('cron:daily_cleanup');
    } catch (e) {
      console.error('[CRONS] Stats/log cleanup error:', e.message || e);
      await releaseLock('cron:daily_cleanup').catch(() => {});
    }
  });

  cron.schedule('*/10 * * * *', async () => {
    try {
      if (!(await acquireLock('cron:auth_flood_cleanup', 120))) {
        console.log('[CRONS] Auth flood cleanup skipped - already running');
        return;
      }
      ...
      await releaseLock('cron:auth_flood_cleanup');
    }
  });

  cron.schedule('*/5 * * * *', async () => {
    try {
      if (!(await acquireLock('cron:import_providers', 300))) {
        console.log(
          '[CRONS] Import provider auto-update skipped - already running'
        );
        return;
      }
      ...
      console.error(
        '[CRONS] Import provider auto-update error:',
        e.message || e
      );
    } finally {
      await releaseLock('cron:import_providers');
    }
  });

  cron.schedule('0 3 * * *', async () => {
    try {
      if (!(await acquireLock('cron:db_optimize', 600))) {
        console.log('[CRONS] DB optimize skipped - already running');
        return;
      }
      await dbService.optimizeDatabase({ source: 'cron' });
      console.log('[CRONS] Daily DB optimize finished');
      await releaseLock('cron:db_optimize');
    } catch (e) {
      console.error('[CRONS] Daily DB optimize error:', e.message || e);
      await releaseLock('cron:db_optimize').catch(() => {});
    }
  });

  cron.schedule('0 */4 * * *', async () => {
    try {
      if (!(await acquireLock('cron:backup', 600))) {
        console.log('[CRONS] Scheduled backup skipped - already running');
        return;
      }
      await backupService.initBackupTable();
      const backup = await backupService.createBackup();
      console.log(`[CRONS] Scheduled backup created: ${backup.filename}`);
      await releaseLock('cron:backup');
    } catch (e) {
      console.error('[CRONS] Scheduled backup error:', e.message || e);
      await releaseLock('cron:backup').catch(() => {});
    }
  });
  ...
  cron.schedule('* * * * *', async () => {
    try {
      ...
      console.error(
        '[CRONS] Stale runtime session cleanup error:',
        e.message || e
      );
    }
  });
  ...
  cron.schedule('*/5 * * * *', async () => {
    try {
      ...
      if (reconciled > 0) {
        console.log(
          `[CRONS] Placement clients reconciled: ${reconciled} placements updated`
        );
      }
    } catch (e) {
      console.error(
        '[CRONS] Placement clients reconciliation error:',
        e.message || e
      );
    }
  });
