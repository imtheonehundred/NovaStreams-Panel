...
function buildMovieProperties(m) {
  ...
  return {
    ...
    youtube_trailer: extractTrailer(m.videos),
  };
}
...
function buildSeriesProperties(s) {
  ...
  return {
    tmdb_id: s.id, title: s.name || s.original_name || '', cover: poster,
    ...
    year: s.first_air_date ? parseInt(s.first_air_date.slice(0, 4)) : null,
    youtube_trailer: extractTrailer(s.videos),
...
module.exports = { searchMovies, searchTvShows, getMovie, getTvShow, getSeason, buildMovieProperties, buildSeriesProperties, getApiKey };
