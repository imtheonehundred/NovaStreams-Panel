...
const { recordSample: bwRecordSample, getLatestSample: bwGetLatestSample } = require('./bandwidthMonitor');
const { isPanelUp, hasPanelHealthSample, getLastCheckAt, getLastResponseMs, getLastError, getConsecutiveFails } = require('./healthMonitor');
...
function buildLocalServerCard({
  ...
  const statusMeta = health.status === 'unknown'
    ? 'Awaiting first check'
    : (lastResponseMs > 0 ? `${lastResponseMs} ms` : 'Realtime');
  ...
  return {
    ...
    metrics: [
      buildDashboardMetric('CPU', system.cpuPct, formatFixed(system.cpuPct, 0, '%')),
      buildDashboardMetric('MEM', system.ramPct, formatFixed(system.ramPct, 0, '%')),
      buildDashboardMetric('IO', ioPct, formatFixed(ioMbps, 1, ' Mbps')),
      buildDashboardMetric('DISK', system.diskPct, formatFixed(system.diskPct, 0, '%')),
    ],
  };
}

function buildRemoteServerCard(server, index, runningByServer, now, formatDuration) {
  ...
  const heartbeatValue = heartbeatAgeMs != null
  ...
  return {
    ...
    statusTone,
    statusText,
    statusMeta: Number.isFinite(ping) ? `${ping.toFixed(0)} ms` : (heartbeatAgeMs != null ? `seen ${heartbeatValue} ago` : 'No telemetry'),
    facts: [
      buildDashboardCardFact('Connections', Number(server.max_clients) > 0 ? `0 / ${server.max_clients}` : '—'),
...
function createWsServer({ getSessionUserId, deps }) {
  ...
  async function broadcastDashboard() {
    try {
      ...
      for (const ch of running) {
        const rawServerId = ch.stream_server_id != null ? ch.stream_server_id : (ch.serverId != null ? ch.serverId : ch.streamServerId);
      ...
      const [m, activeRow, servers] = await Promise.all([
        ...
        queryOne('SELECT COUNT(*) AS c FROM `lines` WHERE admin_enabled = 1 AND exp_date > ?', [nowTs]).catch(() => ({ c: 0 })),
      ...
      // Get health status
      const hasHealthSample = hasPanelHealthSample ? hasPanelHealthSample() : false;
      ...
      const serverCards = [
        buildLocalServerCard({
          ...
          downStreams,
          health: { status: hasHealthSample ? (panelUp ? 'up' : 'down') : 'unknown', lastResponseMs: lastRespMs },
          netInMbps,
          netOutMbps,
          processInfo: {
        ...
        ...servers
          .filter((server) => Number(server.enabled) === 1 && String(server.role || '').toLowerCase() !== 'main')
          .map((server, index) => buildRemoteServerCard(server, index, runningByServer, now, formatDuration)),
      ];
      ...
      const data = {
        ...
        system: {
          ...
          diskUsedGB: m.diskMain ? +((m.diskMain.used || 0) / (1024 * 1024 * 1024)).toFixed(1) : 0,
          diskTotalGB: m.diskMain ? +((m.diskMain.size || 0) / (1024 * 1024 * 1024)).toFixed(1) : 0,
        },
        bandwidth: {
  ...
  function init() {
    ...
    try { require('./healthMonitor').start(); } catch {}
