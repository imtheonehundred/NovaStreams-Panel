...
function isAdultCategory(category) {
  ...
  if (category.is_adult) return true;
  return /\b(xxx|adult|porn)\b/i.test(String(category.category_name || ''));
}
...
async function vodCategories(line) {
  ...
  const cached = typeof redis.cacheGet === 'function' ? await redis.cacheGet(cacheKey) : null;
  if (cached) return cached;

  const result = typeof vodService.listItems === 'function'
    ? await vodService.listItems(null, 100, 0)
    : { movies: [] };
  ...
  if (typeof redis.cacheSet === 'function') {
...
async function seriesCategories(line) {
  ...
  const result = typeof seriesService.listSeries === 'function'
    ? await seriesService.listSeries(null, 100, 0)
    : { series: [] };
