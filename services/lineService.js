...
async function authenticateByAccessToken(token) {
  const line = await dbApi.getLineByAccessToken(String(token));
  if (!line) return null;
  if (line.enabled !== 1 || line.admin_enabled === 0) return null;
  const now = Math.floor(Date.now() / 1000);
  if (line.exp_date != null && line.exp_date !== '' && Number(line.exp_date) < now) return null;
  return line;
}
...
module.exports = {
  durationToSeconds,
  computeExpDateFromPackage,
  applyPackageDefaults,
  bouquetFieldForStreamType,
  createLine,
  authenticateLine,
