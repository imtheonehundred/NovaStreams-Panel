...
describe('FFmpeg Args Library - Extended', () => {
  ...
  describe('needsTranscode', () => {
    it('should return true when outputMode is transcode', () => {
      const channel = { outputMode: 'transcode' };
      expect(needsTranscode(channel)).toBe(true);
    });

    it('should return false when outputMode is copy', () => {
      const channel = { outputMode: 'copy' };
      expect(needsTranscode(channel)).toBe(false);
    });

    it('should return true when watermark is enabled with file', () => {
      const channel = {
        outputMode: 'copy',
        watermark: { enabled: true, file: 'logo.png' }
      };
      expect(needsTranscode(channel)).toBe(true);
    });

    it('should return false when watermark has no file', () => {
      const channel = {
        outputMode: 'copy',
  ...
  describe('buildMergedHeaders', () => {
    it('should return empty object when empty channel', () => {
      expect(buildMergedHeaders({})).toEqual({});
    });

    it('should merge channel headers', () => {
      const channel = {
        headers: { 'X-Custom': 'value' }
      };
      const result = buildMergedHeaders(channel);
      expect(result['X-Custom']).toBe('value');
    });

    it('should add User-Agent when present', () => {
      const channel = {
        headers: {},
    ...
    it('should add Referer when present', () => {
      const channel = {
        headers: {},
        referer: 'http://example.com'
      };
      const result = buildMergedHeaders(channel);
      expect(result['Referer']).toBe('http://example.com');
    });

    it('should override existing headers with channel props', () => {
      const channel = {
        headers: { 'User-Agent': 'original' },
        userAgent: 'override'
      };
      const result = buildMergedHeaders(channel);
      expect(result['User-Agent']).toBe('override');
    });
  });

  describe('minimalIngestEnabled', () => {
    it('should return a boolean', () => {
      expect(typeof minimalIngestEnabled({})).toBe('boolean');
    });

    it('should use channel override when boolean', () => {
      expect(minimalIngestEnabled({ minimalIngest: true })).toBe(true);
      expect(minimalIngestEnabled({ minimalIngest: false })).toBe(false);
    });
  });

  describe('webappIngestStyle', () => {
    it('should return a boolean', () => {
      expect(typeof webappIngestStyle()).toBe('boolean');
    });
  });

  describe('getEffectiveIngestStyle', () => {
    it('should return a string', () => {
      expect(typeof getEffectiveIngestStyle({})).toBe('string');
    });

    it('should return channel override when set', () => {
      const channel = { ingest_style_override: 'xc' };
      expect(getEffectiveIngestStyle(channel)).toBe('xc');
    });
  });
});
