...
if [[ -f "$ENV_FILE" ]] && grep -qE '^LINE_PASSWORD_SECRET=' "$ENV_FILE"; then
  LINE_PASSWORD_SECRET="$(grep -E '^LINE_PASSWORD_SECRET=' "$ENV_FILE" | head -1 | cut -d= -f2-)"
else
  LINE_PASSWORD_SECRET="$(openssl rand -hex 32)"
fi
if [[ -f "$ENV_FILE" ]] && grep -qE '^STREAM_SECRET=' "$ENV_FILE"; then
  STREAM_SECRET="$(grep -E '^STREAM_SECRET=' "$ENV_FILE" | head -1 | cut -d= -f2-)"
else
...
set_env_kv "LINE_PASSWORD_SECRET" "$LINE_PASSWORD_SECRET"
set_env_kv "STREAM_SECRET" "$STREAM_SECRET"
