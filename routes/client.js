...
async function clientAuth(req, res, next) {
  ...
  if (authHeader.startsWith('Bearer ')) {
    ...
    try {
      const line = await lineService.authenticateByAccessToken(token);
      if (line) {
...
function clearClientSession(req) {
  if (!req.session) return;
  req.session.lineId = null;
  req.session.lineUsername = null;
  req.session.lineExpDate = null;
  if (req.session.portalRole === 'user') req.session.portalRole = null;
}

router.get('/csrf-token', clientAuth, (req, res) => {
...
router.get('/me', clientAuth, async (req, res) => {
  try {
    ...
    const line = await queryOne(
      `SELECT l.id, l.username, UNIX_TIMESTAMP(l.exp_date) AS exp_date, l.enabled, l.max_connections,
...
// PUT /client/password
router.put('/password', clientAuth, csrfProtection, async (req, res) => {
  try {
    const lineId = req.session.lineId;
    ...
    const auth = await lineService.authenticateLine(
      lineUsername,
      current_password
    );
...
router.post('/logout', clientAuth, csrfProtection, (req, res) => {
  clearClientSession(req);
  res.json({ ok: true });
});
...
router.get('/connections', clientAuth, async (req, res) => {
  try {
    ...
    const rows = await query(
      `SELECT user_ip AS ip, user_agent,
...
router.get('/playlist', clientAuth, async (req, res) => {
  try {
    ...
    const line = lineService.normalizeLineRow(
      dbApi.attachLinePassword(rawLine)
    );

    const requestBase = `${req.protocol || 'http'}://${req.get('host') || 'localhost'}`;
    const defaultBase = await serverService.resolvePlaylistBaseUrl(
      line,
      requestBase
    );
    const m3u = await playlistService.generatePlaylist(line, {
      ...
      output: 'ts',
      baseUrl: defaultBase,
      resolveBaseUrl: (assetSid) =>
        serverService.resolvePlaylistBaseUrl(line, requestBase, assetSid),
    ...
    res.setHeader(
      'Content-Disposition',
      `attachment; filename="${line.username}.m3u"`
    );
    res.send(m3u);
  } catch (e) {
    res.status(500).json({ error: e.message });
...
router.post('/login', async (req, res) => {
  try {
    ...
    res.json({
      ok: true,
      expired,
      line: { id: line.id, username: line.username },
    });
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
