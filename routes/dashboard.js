...
module.exports = function dashboardRoutes({ channels, processes, userActivity, collectSystemMetrics, dbApi, maxFFmpegProcesses, formatDuration, channelRuntimeInfo }) {
  ...
  router.get('/metrics', async (req, res) => {
    try {
      ...
      const table = mine.map((ch) => {
        const upSec = ch.startedAt ? (now - new Date(ch.startedAt).getTime()) / 1000 : 0;
        return {
          id: ch.id || '',
      ...
      res.json({
        ...
        system: {
          ...
          metricsWarnings: m.warnings && m.warnings.length ? m.warnings : undefined,
