...
module.exports = function drmRoutes({
  ...
  router.post('/drm-restreams/parse-preview', requireAuth, csrfProtection, async (req, res) => {
    try {
  ...
  router.post('/drm-restreams', requireAuth, csrfProtection, async (req, res) => {
    try {
  ...
  router.put('/drm-restreams/:id', requireAuth, csrfProtection, async (req, res) => {
    try {
      const { id } = req.params;
  ...
  router.delete('/drm-restreams/:id', requireAuth, csrfProtection, async (req, res) => {
    try {
      const { id } = req.params;
      const ch = channels.get(id);
      if (!ch || !isInternalChannel(ch)) return res.status(404).json({ error: 'DRM stream not found' });
  ...
  router.post('/drm-restreams/:id/start', requireAuth, csrfProtection, async (req, res) => {
    try {
      const { id } = req.params;
      const ch = channels.get(id);
      if (!ch || !isInternalChannel(ch)) return res.status(404).json({ error: 'DRM stream not found' });
  ...
  router.post('/drm-restreams/:id/stop', requireAuth, csrfProtection, (req, res) => {
    try {
      const { id } = req.params;
      const ch = channels.get(id);
      if (!ch || !isInternalChannel(ch)) return res.status(404).json({ error: 'DRM stream not found' });
