...
router.post('/play/:channelId/start', async (req, res) => {
  ...
  const tokenData = await sessionService.issueToken(user, channelId, req.ip, ttlSec);
...
router.post('/play/:channelId/stop', async (req, res) => {
  const { channelId } = req.params;
  const { token } = req.body || req.query || {};
  if (token) await sessionService.endSession(String(token));
