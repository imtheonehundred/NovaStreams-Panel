  ...
  switch (action) {
    ...
    case 'get_live_streams': {
      ...
      data = await Promise.all(data.map(async (s) => {
        if (!await bouquetAllows(line, s.stream_id, 'live')) return null;
        return s;
      }));
    ...
    case 'get_vod_streams': {
      let data = await xtreamService.vodStreams(line, req.query.category_id, req.query.page, parseInt(req.query.items_per_page, 10) || 50);
      data = await Promise.all(data.map(async (s) => {
        if (!await bouquetAllows(line, s.stream_id, 'movie')) return null;
        return s;
      }));
      data = data.filter(Boolean);
    ...
    case 'get_vod_info': {
      ...
      if (!await bouquetAllows(line, info.vod_id || req.query.vod_id, 'movie')) return res.status(404).json({ error: 'VOD not found' });
      return res.json(info);
    }
    case 'get_series_categories':
      return res.json(paginate(await xtreamService.seriesCategories(line), req));
    case 'get_series': {
      let data = await xtreamService.seriesList(line, req.query.category_id, req.query.page, parseInt(req.query.items_per_page, 10) || 50);
      data = await Promise.all(data.map(async (s) => {
        if (!await bouquetAllows(line, s.series_id, 'series')) return null;
        return s;
      }));
      data = data.filter(Boolean);
      return res.json(data);
    ...
    case 'get_series_info': {
      ...
      if (!await bouquetAllows(line, info.series_id || req.query.series_id, 'series')) return res.status(404).json({ error: 'Series not found' });
      return res.json(info);
    }
    case 'get_short_epg':
