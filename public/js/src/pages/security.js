// pages/security.js - Extracted from modules/security.js
// NovaStreams Panel Security Management Page Module
  ...
  if (tb) {
    tb.innerHTML = roles.map((role) => `
      <tr>
        ...
        <td>
          <button class="btn btn-xs btn-primary" data-app-action="editRolePerms" data-app-args="${role.id}">Permissions</button>
          ${role.id !== 1 ? `<button class="btn btn-xs btn-danger" data-app-action="deleteRole" data-app-args="${role.id}">Del</button>` : ''}
        </td>
...
export async function loadSecurity(ctx) {
  try {
    ...
    ctx.$('#blockedIpsTable tbody').innerHTML = (ips.items || []).map((item) => `
      ...
      <td><button class="btn btn-xs btn-danger" data-app-action="removeBlockedIp" data-app-args="${item.id}">Del</button></td></tr>`).join('');
    ctx.$('#blockedUasTable tbody').innerHTML = (uas.items || []).map((item) => `
      ...
      <td><button class="btn btn-xs btn-danger" data-app-action="removeBlockedUa" data-app-args="${item.id}">Del</button></td></tr>`).join('');
    ...
    ctx.$('#blockedAsnTable tbody').innerHTML = (asnBlocked.blocked || []).map((asn) => `
      <tr>
        ...
        <td><button class="btn btn-xs btn-danger" data-app-action="unblockAsn" data-app-args="'${ctx.escHtml(asn.asn)}'">Unblock</button></td>
    ...
    ctx.$('#multiloginTable tbody').innerHTML = (mlLines.lines || []).map((line) => `
      <tr>
        ...
        <td><button class="btn btn-xs btn-warning" data-app-action="disconnectLine" data-app-args="${line.lineId}">Disconnect</button></td>
...
export async function addBlockedIp(ctx) {
...
export async function addBlockedUa(ctx) {
...
export async function removeBlockedIp(ctx, id) {
...
export async function blockAsn(ctx) {
...
export async function unblockAsn(ctx, asn) {
...
export async function saveMultiloginSettings(ctx) {
...
export async function disconnectLine(ctx, lineId) {
...
export async function deleteRole(ctx, id) {
...
export async function editRolePerms(ctx, roleId) {
  try {
...
export async function saveRolePerms(ctx, roleId) {
