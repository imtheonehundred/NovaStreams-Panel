// pages/monitor.js - Extracted from modules/monitor.js + app.js
// NovaStreams Panel Monitor Page Module with Top Channels Monitoring
...
export async function loadMonitorPage(ctx) {
  try {
    ...
    if (healthData) {
      ...
      if (dot && status) {
        ...
        if (meta) {
          meta.textContent = isUnknown ? 'Awaiting first health check' : (healthData.lastResponseMs ? `Last check: ${healthData.lastResponseMs}ms` : 'System status');
        }
      }
    }
  } catch (e) {
...
export function stopTopChannelsMonitorAutoRefresh(ctx) {
  if (ctx.getTopChannelsMonitorTimer()) {
    clearInterval(ctx.getTopChannelsMonitorTimer());
    ctx.setTopChannelsMonitorTimer(null);
  }
}

export async function loadMonitorTopChannelsPage(ctx) {
  try {
    const data = await ctx.apiFetch('/live-connections/top-channels').catch(() => ({ channels: [] }));
    const tbody = ctx.$('#topChannelsTable tbody');
    if (tbody) {
      const channels = Array.isArray(data.channels) ? data.channels : [];
      tbody.innerHTML = channels.length ? channels.map((ch, idx) => `
        <tr>
          <td>${idx + 1}</td>
          <td>${ctx.escHtml(String(ch.id || ''))}</td>
          <td>${ctx.escHtml(ch.name || '—')}</td>
          <td>${ch.clients || 0}</td>
          <td>${ctx.escHtml(ch.server_name || '—')}</td>
        </tr>`).join('') : '<tr><td colspan="5" class="text-muted">No active streams</td></tr>';
    }
  } catch (e) {
    ctx.toast(e.message, 'error');
  }
