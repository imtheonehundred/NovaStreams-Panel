// NovaStreams Reseller Portal - Vite Entry Point
// Converts reseller-app.js from IIFE to ES6 module format
...
function lineStatusBadge(l) {
  ...
  if (l.admin_enabled === 0)
    return '<span class="badge badge-danger">Banned</span>';
  if (l.exp_date && l.exp_date < now)
...
function applyResellerProfileState(profile) {
  ...
  if (expiryNav)
    expiryNav.style.display =
      profile && Number(profile.manage_expiry_media) === 1 ? '' : 'none';
  const announcementWrap = $('#dashAnnouncement');
  const announcementBody = $('#dashAnnouncementBody');
  const html =
    profile && profile.notice_html ? String(profile.notice_html).trim() : '';
  if (announcementWrap && announcementBody) {
    if (html) {
      const sanitized = sanitizeHtml(html);
      announcementBody.innerHTML = '';
      announcementBody.appendChild(sanitized);
...
function showPanel() {
  ...
  const saved = (() => {
    try {
      return localStorage.getItem('rslLastPage');
    } catch {
      return '';
    }
  })();
  ...
  let page = hash || saved || 'dashboard';
  if (!hash && lastSegment && lastSegment !== pathParts[0]) {
    page = lastSegment;
  }
  navigateTo(page);
}
...
async function doLogin(e) {
  ...
  try {
    ...
    if (data.role && data.role !== 'reseller')
      throw new Error('This account must use admin access code URL');
...
function navigateTo(page) {
  ...
  location.hash = page;
  try {
    localStorage.setItem('rslLastPage', page);
  } catch {}
...
async function loadDashboard() {
  try {
    ...
    const active = lines.filter(
      (l) => l.admin_enabled === 1 && (!l.exp_date || l.exp_date >= now)
    ).length;

    const welcomeEl = $('#dashWelcome');
    if (welcomeEl && credData.username)
      welcomeEl.textContent = 'Welcome, ' + credData.username;
  ...
  try {
    ...
    tbody.innerHTML = filtered
      .map((l) => {
        ...
        <td>
          <button class="btn btn-xs btn-secondary" data-rsl-action="openPlaylistModal" data-rsl-args="'${escHtml(l.username || '')}', '${escHtml(l.password || '')}'">Playlist</button>
          ${canDelete ? `<button class="btn btn-xs btn-danger" data-rsl-action="deleteLine" data-rsl-args="${l.id}">Del</button>` : ''}
        </td>
      </tr>`;
      })
      .join('');
...
function showPackageSummary(pkgId) {
  ...
  const cost = pkg.is_trial
    ? pkg.trial_credits || 0
    : pkg.official_credits || 0;
...
async function saveLine() {
  ...
  try {
    ...
    else if (e.message === 'trial_limit_exceeded')
      toast('Trial generation limit reached for your group', 'error');
    else if (e.message === 'package_forbidden')
      toast('This package is not available to your member group', 'error');
...
async function loadProfile() {
  try {
    ...
    $('#creditLogsTable tbody').innerHTML = logs
      .map(
        (log) => `<tr>
...
function renderExpiryRows(items = []) {
  ...
  if (expiringWrap) {
    ...
    expiringWrap.innerHTML = (rows.length ? rows : [{}])
      .map((item) => buildExpiryRow(item))
      .join('');
  }
  if (expiredWrap) {
    const rows = (items || []).filter((item) => item.scenario === 'expired');
    expiredWrap.innerHTML = (rows.length ? rows : [{}])
      .map((item) => buildExpiryRow(item))
      .join('');
  }
}
...
async function loadExpiryMedia() {
  try {
    ...
    $('#rslExpiryActive').checked =
      !_expiryMediaService || Number(_expiryMediaService.active) === 1;
    $('#rslExpiryWindowDays').value = String(
      (_expiryMediaService && _expiryMediaService.warning_window_days) || 7
    );
    $('#rslExpiryRepeatHours').value = String(
      (_expiryMediaService && _expiryMediaService.repeat_interval_hours) || 6
    );
...
function addExpiryRow(scenario) {
  const wrap =
    scenario === 'expiring'
      ? $('#rslExpiryExpiringRows')
      : $('#rslExpiryExpiredRows');
  if (!wrap) return;
...
function removeExpiryRow(btn) {
  ...
  if (row) row.remove();
  if (wrap && !wrap.querySelector('.input-with-btn'))
    wrap.insertAdjacentHTML('beforeend', buildExpiryRow({}));
}
  ...
  try {
    await apiFetch('/expiry-media', {
      ...
      body: JSON.stringify({
        ...
        warning_window_days:
          parseInt($('#rslExpiryWindowDays').value || '7', 10) || 7,
        repeat_interval_hours:
          parseInt($('#rslExpiryRepeatHours').value || '6', 10) || 6,
        items: [
...
function openPlaylistModal(username, password) {
  ...
  $('#plM3uUrl').value =
    `${base}/get.php?username=${encodeURIComponent(username)}&password=${encodeURIComponent(password)}&type=m3u_plus`;
  $('#plEpgUrl').value =
...
function parseActionArgs(raw, event, element) {
  if (!raw) return [];
  try {
    return Function(
      'event',
...
function bindStaticActionDelegates() {
  document.addEventListener('click', (event) => {
    const navTarget = event.target.closest('[data-rsl-nav]');
    if (navTarget) {
      event.preventDefault();
      navigateTo(navTarget.dataset.rslNav);
      return;
    }

    const actionTarget = event.target.closest('[data-rsl-action]');
    if (!actionTarget) return;
    const action = actionTarget.dataset.rslAction;
    const handler = action && window.RSL && window.RSL[action];
    if (typeof handler !== 'function') return;
    event.preventDefault();
    handler(
      ...parseActionArgs(actionTarget.dataset.rslArgs, event, actionTarget)
    );
  });
}
...
function init() {
  ...
  if (lSearch)
    lSearch.addEventListener('input', () => {
      clearTimeout(lSearch._t);
      lSearch._t = setTimeout(loadLines, 300);
    });

  checkSession();
}

// Expose RSL actions globally for inline onclick handlers (temporary bridge)
window.RSL = {
  navigateTo,
  openLineForm,
  saveLine,
