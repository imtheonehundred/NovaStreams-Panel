(function () {
  ...
  function createServerAreaModule() {
    ...
    async function loadInstallLbPage(ctx) {
    ...
    async function loadInstallProxyPage(ctx) {
      await ctx.getAdminFeatures();
    ...
    async function loadManageProxyPage(ctx) {
      try {
        ...
        const rows = rels.length ? rels.map((r) => `
          <tr>
            ...
            <td>${r.priority || 0}</td>
            <td><button class="btn btn-xs btn-danger" onclick="APP.deleteProxyRelationship(${r.parent_server_id}, ${r.child_server_id})">Remove</button></td>
        ...
        const addForm = `
          <div class="card mt-3">
            ...
            <div class="card-body">
              ...
              <button type="button" class="btn btn-primary" onclick="APP.addProxyRelationship()">Add relationship</button>
            </div>
          </div>`;
    ...
    async function addProxyRelationship(ctx) {
    ...
    async function deleteProxyRelationship(ctx, parentId, childId) {
    ...
    async function loadServerOrderPage(ctx) {
      try {
    ...
    function moveServerOrder(ctx, idx, dir) {
    ...
    async function saveServerOrder(ctx) {
  ...
  root.serverArea = { createServerAreaModule };
}());
